...
static void on_constellation_toggled(GtkToggleButton *button, gpointer user_data) {
    app_state.show_constellations = gtk_toggle_button_get_active(button);
    sky_view_redraw(sky_widget);
}

static void on_time_changed(GtkWidget *widget, gpointer user_data) {
    // Parse time from spin buttons or entries
    // For simplicity, let's just add +/- 1 hour buttons for now?
    // Or just rely on hardcoded time in init?
    // The requirements say "user-specified time".
}

// Timer for auto-refresh if needed, or just manual refresh.
// Let's implement a simple "Time Step" (+1h, -1h)
...
// Callback for sky view selection change
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    // Create Elevation View first as it's needed for sky view callback data?
    // Wait, sky_view_new needs callback. Callback needs global 'elevation_widget' or passed data.
    // I can create the widget but not pack it yet.
    ...
    gtk_container_add(GTK_CONTAINER(frame_left), sky_widget);
    gtk_paned_pack1(GTK_PANED(paned), frame_left, TRUE, FALSE);
    ...
    gtk_box_pack_start(GTK_BOX(box_right), elevation_widget, TRUE, TRUE, 0);

    // Location Controls
    location_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);

    lbl_lat = gtk_label_new("Lat:");
    gtk_box_pack_start(GTK_BOX(location_box), lbl_lat, FALSE, FALSE, 5);
    ...
    gtk_box_pack_start(GTK_BOX(location_box), spin_lat, FALSE, FALSE, 5);

    lbl_lon = gtk_label_new("Lon:");
    gtk_box_pack_start(GTK_BOX(location_box), lbl_lon, FALSE, FALSE, 5);
    ...
    gtk_box_pack_start(GTK_BOX(location_box), spin_lon, FALSE, FALSE, 5);

    gtk_box_pack_start(GTK_BOX(box_right), location_box, FALSE, FALSE, 10);

    // General Controls
    controls_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);

    btn_const = gtk_check_button_new_with_label("Constellations");
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(btn_const), TRUE);
    g_signal_connect(btn_const, "toggled", G_CALLBACK(on_constellation_toggled), NULL);
    gtk_box_pack_start(GTK_BOX(controls_box), btn_const, FALSE, FALSE, 5);

    btn_time_m = gtk_button_new_with_label("-1h");
    g_signal_connect(btn_time_m, "clicked", G_CALLBACK(on_time_sub_1h), NULL);
    gtk_box_pack_start(GTK_BOX(controls_box), btn_time_m, FALSE, FALSE, 5);

    btn_time_p = gtk_button_new_with_label("+1h");
    g_signal_connect(btn_time_p, "clicked", G_CALLBACK(on_time_add_1h), NULL);
    gtk_box_pack_start(GTK_BOX(controls_box), btn_time_p, FALSE, FALSE, 5);

    gtk_box_pack_start(GTK_BOX(box_right), controls_box, FALSE, FALSE, 10);

    gtk_paned_pack2(GTK_PANED(paned), frame_right, TRUE, FALSE);
